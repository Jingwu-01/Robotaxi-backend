...
#     global traci_connection, simulation_running, simulation_parameters
#     try:
...
#                 # Write energy output periodically
#                 if timestep % 50 == 0:
...
#         traci.vehicle.add(vehID=vehicle_id, routeID=selected_route)
...
#     """Updates simulation parameters. Changes take effect during the next simulation run."""
#     data = request.get_json()
#     if not data:
#         return jsonify({'error': 'No input data provided'}), 400
...
def run_simulation():
    """Runs the SUMO simulation using TraCI."""
    global traci_connection, simulation_running, simulation_parameters, cumulative_consumption
    try:
        # Initialize SUMO simulation
        sumo_cfg = SUMO_CONFIG
        step_length = simulation_parameters['time_step']
            ...
            with lock:
                ...
                with cumulative_consumption_lock:
                    for vid in traci.vehicle.getIDList():
                        try:
                            energy_increment = traci.vehicle.getElectricityConsumption(vid)
                            cumulative_consumption[vid] = cumulative_consumption.get(vid, 0.0) + energy_increment
                        except traci.exceptions.TraCIException:
                            # Handle non-electric vehicles or errors
                            pass

                # Write energy output periodically (optional)
                # Uncomment the following lines if you want to keep the energy output files
                # if timestep % 50 == 0:
...
def start_simulation():
    ...
    # Initialize cumulative_consumption
    global cumulative_consumption
    with cumulative_consumption_lock:
        cumulative_consumption = {}
    ...
    try:
        ...
        with cumulative_consumption_lock:
            for vid in vehicle_ids:
                ...
                vehicles[vid] = {"energy_consumption_kJ": energy}
        return jsonify({"vehicles": vehicles}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
